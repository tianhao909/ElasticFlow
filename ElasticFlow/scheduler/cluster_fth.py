...
class _Cluster(object):
    ...
    def __init__(self, num_switch=0, num_node_p_switch=0, num_gpu_p_node=0, num_cpu_p_node=0, mem_p_node=0):
        ...
        self.node_g = {1: self.node_g1, 2:self.node_g2, 4:self.node_g4, 
                ...
                40:self.node_g40, 48:self.node_g48, 56:self.node_g56, 
                64:self.node_g64, 72:self.node_g72, 88:self.node_g88, 128:self.node_g128,
                256:self.node_g256}
    ...
    def init_infra(self, num_switch=0, num_node_p_switch=0, num_gpu_p_node=0, num_cpu_p_node=0, mem_p_node=0):
        ...
        self.future_free_gpus = OrderedDict()
        self.future_free_gpus[0] = self.free_gpu
    ...
    #         utils.print_fn('   YARN scheme is applied')
    #         ret = self.ms_yarn_placement(job)
    #     elif FLAGS.scheme == 'balance':
    #         utils.print_fn('   balance scheme is applied')
    #         ret = 
    #     else:
    #         utils.print_fn('   Default yarn scheme is applied')
    #         ret = self.ms_yarn_placement(job)
    #     if ret == True:
    #         job['status'] = 'RUNNING'
    #     return ret
    ...
    def init_dlas_pack_gpu(self):
        ...
        utils.print_fn("There are %d gpus" % len(self.gpu_list))
    ...
    def init_gandiva_nodes(self):  # 定义初始化 Gandiva 节点的方法
        '''
        init node class:
        '''
        # free nodes
        for switch in self.switch_list:  # 遍历交换机列表
            for node in switch.node_list:  # 遍历每个交换机中的节点列表
                self.free_nodes.append(node)  # 将节点添加到自由节点列表中

        if len(self.free_nodes) != self.num_node:  # 如果自由节点数量不等于总节点数量
            utils.print_fn("free_node: %s, is incorrect" % len(self.free_nodes))  # 打印错误消息
            exit()  # 退出程序

        for num_gpu, node_list in self.node_g.items():  # 遍历存储 GPU 数量和节点列表的字典
            print(num_gpu)  # 打印 GPU 数量
    ...
    def gandiva_placement(self, job):
        ...
        if num_gpu not in self.node_g:
            utils.print_fn("error: job[%d] needs %d GPUs" % (job['job_idx'], num_gpu))
            exit()
        ...
        if len(node_list) > 0:
            ...
            if round(ns_util + job_util, 2) < node_set['capacity']:
                ...
                utils.print_fn("job[%d] starts1" % job['job_idx'])
                return True
            else:
                node_list.append(node_set)
        ...
        if num_node <= len(self.free_nodes):
            ...
            utils.print_fn("job[%d] starts2" % job['job_idx'])
            return True
        ...
        if len(node_list) > 0:
            ...
            node_list.append(node_set)
            node_list.sort(key = lambda e:e.__getitem__('util'))
            utils.print_fn("job[%d] starts3" % job['job_idx'])
            return True
        else: #no existing node_set
            utils.print_fn("job[%d] pend 4, free nodes %d" % (job['job_idx'], len(self.free_nodes)))
            return False
    ...
    def gandiva_node_set_shrink(self, ns_num_gpu, node_list, num_ns, cur_time, jobs, logs):
        ...
        utils.print_fn("node_g%d shrink %d node_sets" % (ns_num_gpu, i))
        return i
    ...
    def gandiva_node_set_expand(self, ns_num_gpu, node_list, num_ns, cur_time, jobs, logs):
        ...
        utils.print_fn("node_g%d expand %d node_sets" % (ns_num_gpu, i))
        return i
    ...
    def time_slicing_execute(self, cur_time, jobs, logs, time_diff):
        ...
        used_gpus = 0
        utils.print_fn("free:%d, 1:%d, 2: %d, 4:%d, 8: %d, 16:%d, 32:%d, 64:%d\n" % (len(self.free_nodes), 
        ...
        if total_nodes != self.num_node:
            utils.print_fn("error")
            exit()
        '''
        # if len(self.node_g1) != len(self.node_g[1]):
        #     utils.print_fn("error: %d" % len(self.node_g[1]))
        #     exit()
        # if len(self.node_g2) != len(self.node_g[2]):
        #     utils.print_fn("error: %d" % len(self.node_g[2]))
        #     exit()
        # if len(self.node_g4) != len(self.node_g[4]):
        #     utils.print_fn("error: %d" % len(self.node_g[4]))
        #     exit()
        # if len(self.node_g8) != len(self.node_g[8]):
        #     utils.print_fn("error: %d" % len(self.node_g[8]))
        #     exit()
        # if len(self.node_g16) != len(self.node_g[16]):
        #     utils.print_fn("error: %d" % len(self.node_g[16]))
        #     exit()
        # if len(self.node_g32) != len(self.node_g[32]):
        #     utils.print_fn("error: %d" % len(self.node_g[32]))
        #     exit()
        # if len(self.node_g64) != len(self.node_g[64]):
        #     utils.print_fn("error: %d" % len(self.node_g[64]))
        #     exit()
        ...
        for num_gpu, node_list in self.node_g.items():
            ...
            for node_set in node_list:
                ...
                utils.print_fn("%d-GPU con:%d, jobs:%d" % (num_gpu, concurrency, len(node_set['jobs'])))
                i = 0
                end_list = list()
                for r_job in node_set['jobs']:
                    if 'overhead' not in r_job:
                        r_job['overhead'] = 0
                    r_job['executed_time'] =  r_job['executed_time'] + max(time_diff - r_job['overhead'], 0)
                    r_job['overhead'] = 0
                    ...
                    if r_job['executed_time'] >= r_job['duration']:
                        ...
                        r_job['end_time'] = cur_time - (r_job['executed_time'] - r_job['duration'])
                        if r_job['end_time'] > r_job['ddl']:
                            utils.print_fn('----job[%d]\'s DDL request is not satisfied. Declined.' % r_job['job_idx'])
                            print(cur_time, "v.s.", r_job['ddl'])
                            JOBS.move_to_declined(r_job)
                ...
                for tmp_job in end_list:
                    ...
                    assert tmp_job not in jobs.running_jobs
        ...
        #     utils.print_fn("1 GPU concurrency %d" % concurrency)
        #     i = 0
        #     end_list = list()
        #     for r_job in node_set['jobs']:
        ...
        #             break
        #         concurrency = concurrency + 1
        #     utils.print_fn("2 GPU concurrency %d" % concurrency)
        ...
        #             break
        #         concurrency = concurrency + 1
        #     utils.print_fn("4 GPU concurrency %d" % concurrency)
        ...
        #             break
        #         concurrency = concurrency + 1
        #     utils.print_fn("8 GPU concurrency %d" % concurrency)
        #     i = 0
        #     end_list = list()
        #     for r_job in node_set['jobs']:
        #         r_job['executed_time'] =  r_job['executed_time'] + 1
        ...
        #     for tmp_job in end_list:
        #         utils.print_fn("job[%d] ends in g8, num_jobs %d" % (tmp_job['job_idx'], len(node_set['jobs'])))
        #         jobs.running_jobs.remove(tmp_job)
        #         node_set['jobs'].remove(tmp_job)
        #         node_set['num_jobs'] = node_set['num_jobs'] - 1
        #         logs.job_complete(tmp_job, cur_time)
        ...
        #         used_gpus = used_gpus - 8
        #         node_release = True
        #         utils.print_fn("node_set release in g8")
        ...
        #     utils.print_fn("16 GPU concurrency %d" % concurrency)
        #     i = 0
        #     end_list = list()
        #     for r_job in node_set['jobs']:
        #         r_job['executed_time'] =  r_job['executed_time'] + 1
        ...
        #     utils.print_fn("32 GPU concurrency %d" % concurrency)
        #     i = 0
        #     end_list = list()
        #     for r_job in node_set['jobs']:
        #         r_job['executed_time'] =  r_job['executed_time'] + 1
        ...
        #     utils.print_fn("64 GPU concurrency %d" % concurrency)
        #     i = 0
        #     end_list = list()
        #     for r_job in node_set['jobs']:
        #         r_job['executed_time'] =  r_job['executed_time'] + 1
    ...
    def random_placement(self, job):    
        ...
        for i in range(len(w_node_list)):
            w_n = w_node_list[i]
            tmp_dict = utils.search_dict_list(node_list, 'node', w_n)
            if tmp_dict == None:
                tmp_dict = dict()
                tmp_dict['node'] = w_n
                tmp_dict['worker'] = 1
        ...
        for i in range(len(ps_node_list)):
            ps_n = ps_node_list[i]
            tmp_dict = utils.search_dict_list(node_list, 'node', ps_n)
            if tmp_dict == None:
                tmp_dict = dict()
                tmp_dict['node'] = ps_n
                tmp_dict['worker'] = 0
    ...
    def check_free_gpu(self):
        ...
        elif FLAGS.scheme == 'elastic':
            free_gpu = 0
            for gpu_num in self.node_g:
                for each_node in self.node_g[gpu_num]:
                    # to be fixed
                    free_gpu += int(each_node['capacity'] - each_node['util'])
            for each_node in self.free_nodes:
                free_gpu += each_node.free_gpus
            return free_gpu
        else:
            free_gpu = 0
            for switch in self.switch_list:
    ...
    """def check_future_free_gpu(self, cur_time):
        if FLAGS.scheme == 'elastic':
            tmp_dict = dict()
            tmp_dict[cur_time] = self.check_free_gpu()
            self.future_free_gpus = tmp_dict

            return self.future_free_gpus
        else:
            return self.future_free_gpus"""
    ...
    release job res
    '''
    def release_job_res(self, job, end=True):
        '''
        ...
        if FLAGS.schedule == 'dlas-gpu-pack':
            ...
            for gpu_idx in job['gpus']:
                ...
                if self.gpu_list[gpu_idx] != 1:
                    utils.print_fn("Error: release gpu error in dlas-gpu-pack\n")
                    job['status'] = 'ERROR'
                    return False
            if end:
                job['status'] = 'END'
        ...
        if FLAGS.scheme == 'elastic':
            print("+++ release job", job['job_idx'])
            node_set = job['node_set']
            del job['node_set']
            node_set['num_jobs'] -= 1
            node_set['util'] -= job['num_gpu']
            node_set['jobs'].remove(job)
            free_gpus_in_node_set = node_set['capacity'] - node_set['util']    
            # move to free nodes or new node list
            assert node_set['util'] >= 0
            if node_set['util'] == 0:
                self.free_nodes += node_set['nodes']
                for each_gpu_num in self.node_g:
                    if node_set in self.node_g[each_gpu_num]:
                        self.node_g[each_gpu_num].remove(node_set)
                self.free_nodes.sort(key = lambda e:e.id)
            elif free_gpus_in_node_set > 0:
                for each_gpu_num in sorted(self.node_g.keys(), reverse=True):
                    if each_gpu_num > free_gpus_in_node_set:
                        continue
                    if free_gpus_in_node_set % each_gpu_num == 0:
                        if free_gpus_in_node_set == node_set['num_gpus']:
                            break
                        # move to a new node list
                        self.node_g[node_set['num_gpus']].remove(node_set)
                        node_set['num_gpus'] = each_gpu_num
                        self.node_g[each_gpu_num].append(node_set)
                        #for each_node_set in self.node_g[each_gpu_num]:
                        #    for each_job in each_node_set['jobs']:
                        #        each_job['node_set'] = node_set
                        self.node_g[each_gpu_num].sort(key = lambda e:e.__getitem__('util'))
                        break

            self.free_gpu = self.check_free_gpu()
            if end:
                job['status'] = 'END'
                utils.print_fn('**** job[%d] completed' % job['job_idx'])
            return True
    ...
    def elastic_placement(self, job):
        '''
        elastic: nodes are put into class(1-GPU, 2-GPU, 4-GPU)
        '''
        ...
        if len(node_list) > 0:
            ...
            ns_util = node_set['util']
            #job_util = round(job['model']['mem_util'], 2)
            job_util = job['num_gpu']
            if int(ns_util + job_util) <= node_set['capacity']:
                node_set['util'] = int(node_set['util'] + job_util)
                node_set['jobs'].append(job)
                node_set['num_jobs'] = node_set['num_jobs'] + 1

                free_gpus_in_node = len(node_set['nodes']) * self.num_gpu_p_node - node_set['util']
                if free_gpus_in_node > 0:
                    for each_gpu_num in sorted(self.node_g.keys(), reverse=True):
                        if each_gpu_num > free_gpus_in_node:
                            continue
                        if free_gpus_in_node % each_gpu_num == 0:
                            # move to a new node list
                            node_set['num_gpus'] = each_gpu_num
                            self.node_g[each_gpu_num].append(node_set)
                            #for each_node_set in self.node_g[each_gpu_num]:
        ...
        #fill in nodes with larger jobs
        for each_gpu_num in reversed(list(self.node_g.keys())):
            if each_gpu_num <= num_gpu:
                break
            other_node_list = self.node_g[each_gpu_num]
            if len(other_node_list) > 0:
                node_set = other_node_list.pop(0)
                ns_util = node_set['util']
                #job_util = round(job['model']['mem_util'], 2)
                job_util = job['num_gpu']
                if int(ns_util + job_util) <= node_set['capacity']:
                    # fill in! and move this node to node_list
                    node_set['util'] = int(node_set['util'] + job_util)
                    node_set['jobs'].append(job)
                    node_set['num_jobs'] = node_set['num_jobs'] + 1

                    free_gpus_in_node = len(node_set['nodes']) * self.num_gpu_p_node - node_set['util']
                    # decide which node list it belong to.
                    if free_gpus_in_node > 0:
                        for each in sorted(self.node_g.keys(), reverse=True):
                            if each > free_gpus_in_node:
                                continue
                            if free_gpus_in_node % each == 0:
                                # move to a new node list
                                #for each_node_set in self.node_g[each]:
                                #    for each_job in each_node_set['jobs']:
                                #        each_job['node_set'] = node_set
                                node_set['num_gpus'] = each
                                job['node_set'] = node_set
                                self.node_g[each].append(node_set)
                                node_list.sort(key = lambda e:e.__getitem__('util'))
                                self.node_g[each].sort(key = lambda e:e.__getitem__('util'))
                                # zhengli nodelist
                                utils.print_fn("job[%d] starts by filling in another node" % job['job_idx'])
                                return True
                    ...
                    other_node_list.append(node_set)
        ...
        if num_node <= len(self.free_nodes):
            ...
            node_set['concurrency'] = 0
            #node_set['capacity'] = int(num_node * self.num_gpu_p_node / num_gpu)
            node_set['capacity'] = num_node * self.num_gpu_p_node
            node_set['num_gpus'] = num_gpu
            node_set['num_jobs'] = 0 
            for i in range(num_node):
                node_set['nodes'].append(self.free_nodes.pop(0))

            #node_set['util'] = int(job['model']['mem_util'])
            node_set['util'] = job['num_gpu']
            node_set['jobs'].append(job)
            node_set['num_jobs'] = 1
        ...
        #no resource, no existing node_set
        utils.print_fn("job[%d] pend 4, free nodes %d" % (job['job_idx'], len(self.free_nodes)))
        print("num_gpu", num_gpu, "free_gpus", self.check_free_gpu())
        #print(self.node_g)
        return False

        #end of elastic schedule and placement
